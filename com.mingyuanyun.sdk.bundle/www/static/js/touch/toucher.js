...
$.fn.toucher = function(opt) {
  ...
  $(document).on('touchstart', opt.el, function(e) {
    eventMark = e;
  ...
  $(document).on('touchmove', opt.el, function(e) {
    eventMark = e;
    ...
    if (
      x1 - x2 > 9 &&
      Math.abs(y1 - y2) < 20 &&
      Math.atan(Math.abs(y1 - y2) / Math.abs(x1 - x2)) < 0.4
    ) {
      // 断定此次事件为移动手势
  ...
  /* $(document).on('touchend',opt.el,function (e) {
        //isActive = false;
...
(function(global, doc, factoryFn) {
  ...
  // 是否含有某个 class
  var hasClass = supports_classList
  ...
  function eventListener(DOM) {
    ...
    function touchend(e) {
      ...
      // 若未监听doubleTap，直接响应
      if (
        !this_touch._events.doubleTap ||
        this_touch._events.doubleTap.length == 0
      ) {
        isSingleTap();
    ...
    function touchmove(e) {
      ...
      x2 = e.touches[0].pageX;
      y2 = e.touches[0].pageY;
      if (Math.abs(x1 - x2) > 9 || Math.abs(y1 - y2) > 9) {
        // 断定此次事件为移动手势
        var direction = swipeDirection(x1, x2, y1, y2);
        EMIT.call(this_touch, 'swipe' + direction, e);
      } else {
