cordova.define("com.mysoft.sqliteplus.SQLitePlugin", function(require, exports, module) {
function getNewSqlitePlugin() {
  var DB_STATE_INIT, DB_STATE_OPEN, newSQLError, nextTick, openDBs;
  ...
  function SQLitePlugin(dbPath) {
    if (!dbPath) {
      throw newSQLError("dbPath must not be empty");
    }
    var self = this;
    self.dbPath = dbPath;

    self.open = function (success, error) {
      if (self.dbPath in openDBs) {
        console.log('database already open: ' + self.dbPath);
        nextTick((function () {
          return function () {
            success(self);
          };
        ...
        console.log('OPEN database: ' + self.dbPath);

        var opensuccesscb = (function () {
          return function () {
            console.log('OPEN database: ' + self.dbPath + ' - OK');
            if (!openDBs[self.dbPath]) {
              console.log('database was closed during open operation');
            }
            if (self.dbPath in openDBs) {
              openDBs[self.dbPath] = DB_STATE_OPEN;
            }
            if (!!success) {
              success(self);
            }
        ...
        var openerrorcb = (function () {
          return function () {
            console.log('OPEN database: ' + self.dbPath + ' FAILED, aborting any pending transactions');
            if (!!error) {
              error(newSQLError('Could not open database'));
            }
            delete openDBs[self.dbPath];
          };
        })();

        openDBs[self.dbPath] = DB_STATE_INIT;
        cordova.exec(opensuccesscb, openerrorcb, "SQLitePlugin", "open", [self.dbPath]);
      }
    }

    self.close = function (success, error) {
      if (self.dbPath in openDBs) {
        console.log('CLOSE database: ' + self.dbPath);
        delete openDBs[self.dbPath];
        cordova.exec(success, error, "SQLitePlugin", "close", [self.dbPath]);
      } else {
    ...
    self.executeSql = function (sql, args, success, error) {
      var executeSuccess = function (result) {
      ...
      cordova.exec(executeSuccess, error, "SQLitePlugin", "executeSql", [self.dbPath, sql, args]);
    };

    self.executeSqlInTransaction = function (statements, success, error) {
      cordova.exec(success, error, "SQLitePlugin", "executeSqlInTransaction", [self.dbPath, statements]);
    };
  };


  var sqlitePlugin = {
    openDatabase: function (dbPath, success, error) {
      dbPath = dbPath.indexOf("file://") == 0 ? dbPath.substr(7, dbPath.length - 1) : dbPath;
      var db = new SQLitePlugin(dbPath);
      db.open(success, error);
      return db;
    },
    deleteDatabase: function (dbPath, success, error) {
      dbPath = dbPath.indexOf("file://") == 0 ? dbPath.substr(7, dbPath.length - 1) : dbPath;
      delete openDBs[dbPath];
      cordova.exec(success, error, "SQLitePlugin", "delete", [dbPath]);
    },
    test: function () {
      console.log('newSqlitePlugin');
    }
  }
  return sqlitePlugin;
}

function getOldSqlitePlugin() {
  var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, SQLiteFactory, SQLitePlugin, SQLitePluginTransaction, SelfTest, argsArray, dblocations, iosLocationMap, newSQLError, nextTick, txLocks;
  ...
  SQLitePlugin.prototype.open = function (success, error) {
    ...
    if (this.dbname in this.openDBs) {
      ...
      step2 = (function (_this) {
        return function () {
          cordova.exec(opensuccesscb, openerrorcb, "OldSQLitePlugin", "open", [_this.openargs]);
        };
      })(this);
      cordova.exec(step2, step2, "OldSQLitePlugin", "close", [{
        path: this.dbname
      }]);
    }
  };
  ...
  SQLitePlugin.prototype.close = function (success, error) {
    if (this.dbname in this.openDBs) {
      ...
      cordova.exec(success, error, "OldSQLitePlugin", "close", [{
        path: this.dbname
      }]);
    } else {
      console.log('cannot close: database is not open');
  ...
  SQLitePluginTransaction.prototype.run = function () {
    ...
    cordova.exec(mycb, null, "OldSQLitePlugin", "backgroundExecuteSqlBatch", [{
      dbargs: {
  ...
  SQLiteFactory = {
    ...
    openDatabase: argsArray(function (args) {
      ...
      if (typeof args[0] === "string") {
        var dbFullPath = args[0].indexOf("file://") == 0 ? args[0].substr(7, args[0].length - 1) : args[0];
        var dbName = dbFullPath.split("/").pop();
        var dbPath = dbFullPath.substr(0, dbFullPath.indexOf(dbName) - 1);
        openargs = {};
        openargs.name = dbName;
        openargs.dblocation = dbPath;
    ...
    deleteDatabase: function (first, success, error) {
      ...
      if (first.constructor === String) {
        ...
        var dbFullPath = first.indexOf("file://") == 0 ? first.substr(7, first.length - 1) : first;
        var dbName = dbFullPath.split("/").pop();
        var dbPath = dbFullPath.substr(0, dbFullPath.indexOf(dbName) - 1);
        args.path = dbName;
        args.dblocation = dbPath;
      ...
      return cordova.exec(success, error, "OldSQLitePlugin", "delete", [args]);
    }
  };
  ...
  var sqlitePlugin = {
    ...
    echoTest: function (okcb, errorcb) {
      ...
      return cordova.exec(ok, error, "OldSQLitePlugin", "echoStringValue", [{
        value: 'test-string'
      }]);
    },
...
var oldSqlitePlugin = null;
var newSqlitePlugin = null;

window.sqlitePlugin = {
  openDatabase: function (dbPath, success, error, old) {
    var db;
    if (old) {
      if (!oldSqlitePlugin) {
        oldSqlitePlugin = getOldSqlitePlugin();
      }
      db = oldSqlitePlugin.openDatabase(dbPath, success, error);
    } else {
      if (!newSqlitePlugin) {
        newSqlitePlugin = getNewSqlitePlugin();
      }
      db = newSqlitePlugin.openDatabase(dbPath, success, error);
    }
    return db;
  },
  deleteDatabase: function (dbPath, success, error, old) {
    if (old) {
      if (!oldSqlitePlugin) {
        oldSqlitePlugin = getOldSqlitePlugin();
      }
      oldSqlitePlugin.deleteDatabase(dbPath, success, error);
    } else {
      if (!newSqlitePlugin) {
        newSqlitePlugin = getNewSqlitePlugin();
      }
      newSqlitePlugin.deleteDatabase(dbPath, success, error);
    }
  },
}
});
