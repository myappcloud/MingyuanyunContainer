 ...
 * =====================================================
 */
/* 判断当前device是否支持SUI的路由方法 */
var supportSUIRouter =
  'state' in history && 'pushState' in history && 'replaceState' in history;
/* 不支持则禁用SUI的路由 */
if (!supportSUIRouter) {
  $(function() {
    $('html').addClass('xt-module-low');
  });
  $.config = { router: false };
}
...
(function($) {
  ...
  $.cancelAnimationFrame = function(id) {
    ...
    // else if (webkitCancelRequestAnimationFrame) return webkitCancelRequestAnimationFrame(id);
    else {
 ...
 * FastClick
 * uupdate 20180930
 * github 提交code 93ed42a
 * 目前这个问题主要还是苹果11.3+系统引起的问题，FastClick 官方并没有解决这个问题到master 也没有发布版本
 * 不要问我为什么FastClick在这个文件里面
 * form 云客
 * https://github.com/yunkeCN/fastclick/tree/%40yued/fastclick
 * */
(function() {
  'use strict';
...
+(function($) {
  ...
  $.openModal = function(modal, cb) {
    ...
    var isNotToast = !modal.hasClass('toast');
    if (
  ...
  var Calendar = function(params) {
    ...
    var defaults = {
      ...
      toolbarTemplate:
        ...
        '<span class="clear-calendar">清空</span>' +
        '{{monthPicker}}' +
        '{{yearPicker}}' +
        '<span class="close-panel">确定</span>' +
    ...
    p.initCalendarEvents = function() {
      ...
      p.container.find('.close-panel').on('click', function() {
        $.closeModal(p.container);
      });
      p.container.find('.clear-calendar').on('click', function() {
        p.setValue('');
      });
    ...
    p.monthHTML = function(date, offset) {
      ...
      var daysInMonth = p.daysInMonth(date);
      var firstDayOfMonthIndex = new Date(
        date.getFullYear(),
        date.getMonth()
      ).getDay();
      ...
      var cols = 7;
      var monthHTML = '';
      var dayIndex = 0 + (p.params.firstDay - 1);
      var today = new Date().setHours(0, 0, 0, 0);
      var minDate = p.params.minDate
        ? new Date(p.params.minDate).getTime()
        : null;
      var maxDate = p.params.maxDate
    ...
    function openOnInput(e) {
      ...
      if (p.params.scrollToInput) {
        ...
        var paddingBottom = parseInt(pageContent.css('padding-bottom'), 10);
        var pageHeight =
...
+(function($) {
  ...
  var defaults = {
    ...
    onChange: function(picker, values, displayValues) {
      ...
      var currentValue = picker.cols[4].value;
      if (currentValue > days.length) currentValue = days.length;
    ...
    onOpen: function(picker) {
      if (picker.input.val() === '') {
        var value = picker.params.value.join('-');
    ...
    function handleTouchMove(e) {
      ...
      if ((touchesDiff > 0 && scrollTop <= 0) || scrollTop < 0) {
        ...
        if (useTranslate) {
          ...
          container.transform('translate(0,' + translate + 'px)');
        } else {
        }
        if (
...
+(function($) {
  ...
  $(function() {
    ...
    $(document).on('click', '.searchbar-cancel', function(e) {
      var $btn = $(e.target);
      var $parents = $btn.parents('.searchbar');
      if (!$parents.hasClass('always-active')) {
        $parents.removeClass('searchbar-active');
      }
    });
    $(document).on('blur', '.searchbar input', function(e) {
      var $input = $(e.target);
      var $parents = $input.parents('.searchbar');
      if (!$parents.hasClass('always-active')) {
        $parents.removeClass('searchbar-active');
      }
    });
...
+(function($) {
  ...
  var Util = {
    ...
    toUrlObject: function(url) {
      ...
      var baseUrl = this.getBaseUrl(fullUrl);
      var fragment = this.getUrlFragment(url);
  ...
  Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
    ...
    var $newDoc = $(
      $('<div></div>')
        .append(this.cache[urlObj.base].$content)
        .html()
    );
  ...
  $(function() {
    // 用户可选关闭router功能
    // if (!$.smConfig.router || $.device.android) {
    if (!$.smConfig.router) {
      return;
    }
...
(function($) {
  function Router() {
    var me = this;
    var $currentPage = $('.page.' + routerConfig.curPageClass);
    if ($currentPage.size() == 0) {
      $currentPage = $('.page').eq(0);
      $currentPage.addClass(routerConfig.curPageClass);
    }
    var currentPageId = $currentPage.attr('id');
    if (!currentPageId) {
      currentPageId = 'page-' + +new Date();
      $currentPage.attr('id', currentPageId);
    }

    // history.replace(location.href+='#'+currentPageId);
    // me.table.push(currentPageId);
    $(window).on('hashchange', function(e) {
      // if(me.record.length==0) return;
      var to = Util.getUrlFragment(e.newURL);
      var from = Util.getUrlFragment(e.oldURL);
      // if(!from){from=$('.page-current').attr('id')};
      if (!to) {
        to = $currentPage.attr('id');
      }
      $('.page').removeClass('page-current');
      $('#' + to).addClass('page-current');

      /*  if (from == '') {
             return;
             }
             var direction = null;
             var toPageIndex = $.inArray(to, me.table);
             var fromPageIndex = $.inArray(from, me.table);
             if (toPageIndex >= 0) {
             direction = 'back';
             me.table.splice(fromPageIndex, 1);
             } else {
             me.table.push(to);
             }
             me._go(from, to, direction); */
    });
  }
  ...
  var Util = {
    ...
    setHash: function(pageId) {
      ...
      location.href = this.getBaseUrl(location.href) + pageId;
    },
  };
  ...
  Router.prototype._go = function(from, to, direction) {
    var $curPage = $('#' + from);
    var $newPage = $('#' + to);
    ...
    var aniDirection = DIRECTION.rightToLeft;
    if (direction == 'back') {
      aniDirection = DIRECTION.leftToRight;
    }

    this._animateSection($curPage, $newPage, aniDirection);
    // this._pushNewState('#' + sectionId, sectionId);
    // Util.setHash(sectionId);
  };
  ...
  $(function() {
    if (!supportSUIRouter) {
      // if(!supportSUIRouter || $.device.android){
      $.router = new Router();
      $(document).on('click', 'a', function(e) {
        var $this = $(this);
...
/* SUI datePicker ios穿透滑动 */
(function($) {
  $('body').on('touchmove', '.picker-modal > header', function(event) {
    event.preventDefault();
  });
  $('body').on('touchmove', '.picker-items-col-divider', function(event) {
    event.preventDefault();
  });
})(Zepto);
